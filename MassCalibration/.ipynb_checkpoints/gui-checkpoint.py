...
class GUI:
    ...
    def draw_me_vs_tof(self, label_offset=5, x_min=None, x_max=None, y_min=None, y_max=None):
        ...
        # Create TGraphErrors for sigmaRevT vs TOF
        graph_sigmaRevT_RevT = ROOT.TGraphErrors()
        graph_sigmaRevT_RevT.SetTitle(f"sigmaRevT vs RevT")
        graph_sigmaRevT_RevT.SetMarkerStyle(20)  # Square markers
        graph_sigmaRevT_RevT.SetFillColor(1)
        graph_sigmaRevT_RevT.SetLineColor(1)
        graph_sigmaRevT_RevT.SetLineWidth(2)
        graph_sigmaRevT_RevT.SetMarkerColor(1)
        ...
        for ion in self.data.ions:
            if ion.exp_mass_excess is not None and ion.mass_excess is not None:
                ...
                graph_sigmaRevT_RevT.SetPoint(i, tof, sigmaRevT)
                graph_sigmaRevT_RevT.SetPointError(i, 0, tof_error)
                i += 1
        ...
        canvas = ROOT.TCanvas("canvas_me_vs_tof", "canvas_me_vs_tof", 910, 1100)
        ...
        canvas.Divide(1,2)
        canvas.cd(1)
        gPad.SetBottomMargin(0.12)  # Adjusted margin to ensure x-axis title is not blocked
        ...
        graph_me.GetYaxis().SetTitleOffset(0.8)
        ...
        canvas.cd(2)
        gPad.SetBottomMargin(0.12)  # Adjusted margin to ensure x-axis title is not blocked
        graph_sigmaRevT_RevT.Draw("APE")
        graph_sigmaRevT_RevT.GetXaxis().SetTitle("Revolution time (ns)")
        graph_sigmaRevT_RevT.GetXaxis().SetLimits(x_min, x_max)
        graph_sigmaRevT_RevT.GetXaxis().SetNdivisions(505)
        graph_sigmaRevT_RevT.GetXaxis().CenterTitle(True)
        graph_sigmaRevT_RevT.GetXaxis().SetLabelFont(42)
        graph_sigmaRevT_RevT.GetXaxis().SetLabelSize(0.06)
        graph_sigmaRevT_RevT.GetXaxis().SetTitleSize(0.06)
        graph_sigmaRevT_RevT.GetXaxis().SetTitleFont(42)
        graph_sigmaRevT_RevT.GetYaxis().SetTitle("#sigma_{T} [ps]")
        graph_sigmaRevT_RevT.GetYaxis().CenterTitle(True)
        graph_sigmaRevT_RevT.GetYaxis().SetNdivisions(510)
        graph_sigmaRevT_RevT.GetYaxis().SetLabelFont(42)
        graph_sigmaRevT_RevT.GetYaxis().SetLabelSize(0.06)
        graph_sigmaRevT_RevT.GetYaxis().SetTitleSize(0.06)
        graph_sigmaRevT_RevT.GetYaxis().SetTitleFont(42)
        graph_sigmaRevT_RevT.GetYaxis().SetTitleOffset(0.8)
