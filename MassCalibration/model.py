...
class Model:
    ...
    # 将 NumPy 矩阵转换为 mpmath 矩阵
    def np_to_mpmath(self,matrix):
        return mpmath.matrix(matrix.tolist())

    # 将 mpmath 矩阵转换为 NumPy 矩阵
    def mpmath_to_np(self,matrix):
        return np.array(matrix.tolist(), dtype=np.float64)
    
    def list_to_mpmath(self, matrix):
        return mpmath.matrix(matrix)
        ...
        for ion in self.data.ions:
            if not ion.is_reference_ion:
                ...
                ion_data = [
                    ...
                    ["Exp_Mass_Q", f"{float(ion.exp_mass_Q):.5e}"],
                    ["Exp_Mass_Q_Error", f"{float(ion.exp_mass_Q_error):.5e}"],
                    ["Mass_N (MeV)", f"{float(Mass_N):.5e}"],
                    ["Exp_Mass_Excess (MeV)", f"{float(ion.exp_mass_excess):.5e}"],
                    ["Mass_Excess(exp-ame) (keV)", f"{(float(ion.exp_mass_excess)-ion.mass_excess)*1000:.5e}"],
                    ["Exp_Mass_Excess_Error (keV)", f"{float(ion.exp_mass_excess_error) * 1000:.5e}"]
                ]
