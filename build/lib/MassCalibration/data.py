    ...
    def read_revolution_time(self, file_path):
        ...
        for line in lines:
            ...
            if line:
                ...
                ion.set_revolution_time(revolution_time, revolution_time_error)
    ...
    def print_final_experimental_me(self):
        ...
        for ion in self.get_ions():
            if ion.exp_mass_excess is not None and ion.mass_excess is not None:
                ...
                print(f"{ion_name:<8} {'Y' if ion.is_reference_ion else 'N':<8} "
                      f"{revolution_time:<15.5f} {exp_me_diff:<10.3f} {exp_mass_excess:<10.3f} "
                      f"{exp_mass_excess_error:<10.3f} {mass_excess_error:<10.3f} keV")
        if self.chi2_me is not None and self.sigma_syst_me is not None:
            print(f"Chi2 Mass Excess: {float(self.chi2_me):.2f}, Systematic error Mass Excess: {float(self.sigma_syst_me) * 1000:.2f} keV")
